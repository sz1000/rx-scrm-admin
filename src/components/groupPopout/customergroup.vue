  ...
  <div>
    <div>
      <el-dialog
        ...
        :before-close="groupVisibleCancel"
      ...
      >
        <div>
          <ul>
            <li
              class="label_warp"
              v-for="itemlist in customerstagList"
              :key="itemlist.id"
            >
              <div class="label_tite">{{ itemlist.name }}</div>
              <div class="label_box">
                <span
                  v-for="(itemchildren, index) in itemlist.children"
                  :key="index"
                  class="label_name"
                  :class="{
                    active_name:
                  ...
                  @click="getLabelName(itemchildren, index)"
...
export default {
  ...
  methods: {
    ...
    //   点击取消
    groupVisibleCancel() {
      this.$emit("groupVisibleFrom", this.namelabutArr);
      this.getTagList();
    },
    // 点击确定
    confirmVisible() {
      this.$emit("groupVisibleFrom", this.highLightArr, this.tagidList);
      //   this.$emit("groupVisibleFrom", this.list);
      console.log(this.highLightArr, "------------------this.highLightArr");
    ...
    getLabelName(list, index) {
      ...
      // console.log(this.highLightArr, "获取标签内容");
      // console.log(this.labename.tagid);
    },
    // 点击删除标签
  },
};
