<template>
  <div class="add_filed">
    <el-dialog title="修改字段" :visible.sync="showEdit" :before-close="dialogFormCancel" :close-on-click-modal='false' width="520px">
      <el-form :model="addCodeForm" :rules="editRules" ref="ruleForm" label-width="100px" label-position="right">
        ...
        <el-form-item v-if="addCodeForm.radio == '2'">
          <div class="tagName" v-for="(item,index) in addCodeForm.tagList" :key="index">
            <el-input v-model.trim="item.columnOption" placeholder="请输入" @change="changeItem(item)"></el-input>
...
export default {
  ...
  created() {
    console.log('------', this.totalList, this.detailObj)
    const { columnType, columnName, columnsOptionsVOList } = this.detailObj
    this.addCodeForm.content = columnName
    this.addCodeForm.radio = columnType
    this.addCodeForm.tagList = columnsOptionsVOList
  },
  watch: {},
  methods: {
    dialogFormCancel() {
