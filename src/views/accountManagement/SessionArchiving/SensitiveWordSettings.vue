      ...
      <template v-if="enterTable && enterTable.length">
        <div v-for="(i, index) in enterTable" :key="index" class="list-box">
          ...
          <div class="middle">
            ...
            <el-input v-show="tagIndex == index" class="item add-input" v-model.trim="tagName" placeholder="输入后按回车完成"
                      @keyup.enter.native="doAddTag(i.id)" maxlength="32"></el-input>
...
import {
  ChatSensitiveWordsList,
  ChatSensitiveWordsSyn,
  AddChatSensitiveWordsItem,
  DeleteChatSensitiveWords,
} from '../../../config/api'
