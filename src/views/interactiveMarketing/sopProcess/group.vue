  ...
  <div class="list_wrap">
    <div class="search_wrap">
      <div class="search_box">
        ...
        <div class="search_item">
          ...
          <div class="val">
            <el-select class="select" v-model="search.promptRule" placeholder="请选择规则">
              <el-option v-for="item in promptRuleList" :key="item.id" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </div>
        </div>
        ...
        <div class="search_item">
          ...
          <div class="val">
            <el-date-picker class="select" v-model="creatDate" @change="timeChangeFun" type="daterange" range-separator="-" start-placeholder="开始日期"
    ...
    <div class="content">
      <div class="btn_box" v-show="expandedKeys.some(item=>item.enName == 'add')">
        <div class="btn_add" @click="$router.push({path:'/interactivemarketing/sopProcess/groupAdd',query: {menuFlag: true,type: 1}})">
      ...
      <div class="table">
        <el-table :data="list" :header-cell-style="{ background: '#FAFBFF',color: '#3C4353' }" style="width: 100%">
          ...
          <el-table-column prop="promptRule" label="推送规则">
            <template slot-scope="scope">
              {{scope.row.promptRule == '0' ? '定时推送' : '周期推送'}}
            </template>
          </el-table-column>
          <el-table-column prop="groupCount" label="执行群聊数"></el-table-column>
          <el-table-column prop="createUserName" label="创建人员"></el-table-column>
          <el-table-column prop="createTimeStr" label="创建时间" min-width="160"></el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="180">
            <template slot-scope="scope">
              <el-button type="text" v-show="expandedKeys.some(item=>item.enName == 'edit')" size="medium" @click="editFun(scope.row)">编辑</el-button>
              <el-button type="text" size="medium" @click="toDetail(scope.row)">详情</el-button>
              <el-button type="text" v-show="expandedKeys.some(item=>item.enName == 'delete')" size="medium" @click="deleteDialog(scope.row)">删除
              </el-button>
            </template>
          </el-table-column>
      ...
      <div class="pagePosition">
        <Pagination :total="total" :pageInfo="pageInfo" @pageChange="pageChange" v-if="total">
        </Pagination>
      </div>
    </div>
    <!-- 删除弹窗 -->
...
export default {
  ...
  data() {
    return {
      ...
      options: [],
      search: {
        sopType: 1, //SOP类型:0-个人SOP，1-群SOP
        ruleName: '',
        groupId: '',
        promptRule: '',
        createUserId: '',
  ...
  methods: {
    ...
    editFun(row) {
      //编辑
      this.$router.push({
        path: '/interactivemarketing/sopProcess/groupAdd',
        query: {
          id: row.id,
        },
      })
    ...
    toDetail(row) {
      //详情
      this.$router.push({
        path: '/interactivemarketing/sopProcess/groupDetail',
        query: {
          menuFlag: true,
          type: 1,
          id: row.id,
