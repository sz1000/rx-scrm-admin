  ...
  <div class="list_wrap">
    ...
    <div class="content">
      <div class="btn_box" v-show="expandedKeys.some(item=>item.enName == 'add')">
        <div class="btn_add" @click="$router.push({path:'/interactivemarketing/sopProcess/personalAdd',query: {menuFlag: true,type: 1}})">
        ...
        <el-table :data="list" :header-cell-style="{ background: '#FAFBFF',color: '#3C4353' }" style="width: 100%">
          ...
          <el-table-column fixed="right" label="操作" width="180">
            <template slot-scope="scope">
              ...
              <el-button type="text" size="medium" v-show="expandedKeys.some(item=>item.enName == 'delete')" @click="deleteDialog(scope.row)">删除
              </el-button>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    editFun(row) {
      //编辑
      this.$router.push({
        path: '/interactivemarketing/sopProcess/personalAdd',
        query: {
          id: row.id,
        },
      })
    ...
    toDetail(row) {
      //详情
      this.$router.push({
        path: '/interactivemarketing/sopProcess/personalDetail',
        query: {
          menuFlag: true,
          type: 1,
          id: row.id,
