<template>
  ...
  <div v-if="newAddTech" class="warpList" v-loading="loading">
    <ul class="radio-type">
      ...
      <li v-for="(item, index) in radioList" :key="index" @click="changeRadio(item, index)">
        <span :class="{ active: itemInfo.activeIndex == item.value }" class="defaultBorder"></span>
        <span :class="{ activeTxt: itemInfo.activeIndex == item.value }" class="defaultColor">{{ item.name }}</span>
      </li>
      <!--  -->
    ...
    <div class="textarea-box">
      ...
      <section class="upload-box" v-show="
          itemInfo.activeIndex == 'image' || itemInfo.activeIndex == 'text'
        ">
        <el-upload class="avatar-uploader" action="http://172.10.7.200:8099/customer-service/oss/uploadfileparam" ref="upload" :auto-upload="false"
                   :show-file-list="true" :file-list="fileList" :on-change="changeFile" :on-remove="removeFile" :before-upload="beforeAvatarUpload">
          <img v-if="itemInfo.imageUrl" :src="itemInfo.imageUrl" class="avatar" />
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
        </el-upload>
        <div class="adviceTxt" v-show="
            itemInfo.activeIndex == 'image' || itemInfo.activeIndex == 'text'
          ">
      ...
      <section class="link-box" v-show="itemInfo.activeIndex == 'url'">
        ...
        <el-form>
          <el-form-item class="bootm">
            <!-- <el-input
              v-model="itemInfo.url"
              @blur="checkUrl(itemInfo.url)"
              placeholder="链接地址请以http或https开头"
            ></el-input>  -->
            <el-input v-model="itemInfo.url" @input="
                itemInfo.url = itemInfo.url.replace(
                  /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]|[^\S+$]/g,
                  ''
                )
              " placeholder="链接地址请以http或https开头" @blur="blurUrl"></el-input>
          </el-form-item>
          <!-- <el-form-item prop="skipUrl">
            <el-input
              v-model="fromLis.skipUrl"
              placeholder="链接地址请以http或https开头"
            >
            </el-input>
          </el-form-item> -->
            ...
            <div class="link_cover">
              ...
              <span class="uploadLink" v-if="!this.itemInfo.linkImgUrl">
                <el-upload class="upload-demo" ref="upload" drag action="#" :http-request="handleChange" :file-list="linkImgList"
                           :on-remove="handleImgRemove" :limit="1" :on-change="changeFileurl">
      ...
      <section class="material_warp border_none" v-show="itemInfo.activeIndex == 'sucai'">
        <MaterialDialog @sure="fnMaterialInfo" :dialogVisible="openDialog" ref="mater"></MaterialDialog>
        <div class="mater_content">
          <div class="articleShow" v-show="materObj.tab == 1">
            ...
            <div class="card_article" v-show="materObj.tab == 1 && materObj.title">
          ...
          <div class="photoSHow" v-show="materObj.tab == 3">
            ...
            <div class="photo_card">
              ...
              <span class="txt_btn" @click="resetSlect" v-show="materObj.tab == 3 && materObj.posterUrl">重新选择</span>
            </div>
          </div>
        </div>
      </section>
...
export default {
  ...
  created() {
  //  let allLists =  JSON.parse(JSON.stringify(this.allList))
  ...
  methods: {
    ...
    blurUrl() {
      console.log(1)
      if (
        !this.$httpReg(this.itemInfo.url) ||
        !this.$linkReg(this.itemInfo.url)
      ) {
    ...
    fnMaterialInfo(obj) {
     let objs =  JSON.parse(JSON.stringify(obj))
      ...
      this.itemInfo.mediaDesc = objs.contentAbstract
      this.itemInfo.meiapicul = objs.cover
      this.itemInfo.mediatite = objs.title
      this.itemInfo.mediatype = objs.tab
      this.itemInfo.objList = objs
          if(objs.tab == 3 ){
              // this.itemInfo.meiapicul = obj.posterUrl
             this.itemInfo.mediatite = objs.posterName
             this.itemInfo.urls = objs.posterUrl
             this.itemInfo.mediatype = objs.tab
       }
       if(objs.tab == 2){
           this.itemInfo.mediatite = objs.name
           this.itemInfo.mediatype = objs.tab
       }
        ...
        this.$network
          ...
          .then((res) => {
            ...
            if (res.result) {
              ...
              this.itemInfo.text = res.data.name
              // this.$message({
              //   message: "上传图片成功",
              //   type: "success",
              // });
    ...
    deleteIndex(index) {
      ...
      // let fnDelete =  JSON.parse(JSON.stringify(this.allList))
      // this.$emit('fnDelete',   this.allList)
