    ...
    <el-card>
      <div style="width: 810px">
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          ...
          <el-form-item class="groupListwind" label="选择群发员工: " prop="region" label-width="120px">
            <el-select multiple v-model="ruleForm.region" placeholder="请选择群发员工（可多选）" v-loadmore="loadMore" @visible-change="groupStaffset"
                       @change="groupStaff">
          ...
          <el-form-item label="选择群发客户: " prop="resource" label-width="120px" class="groupListwind">
            ...
            <div v-if="ruleForm.resource == 1">
              <div class="screen_group">
                <el-form-item label="性别:">
                  <el-radio-group v-model="ruleForm.gender" @change="radioGender">
                ...
                <el-form-item label="所在群聊: ">
                  <el-select class="group_name" v-model="ruleForm.group" multiple placeholder="请选择所在群聊 （可多选）" @change="groupchat"
                             v-loadmore="loadMoreGroup">
                    <!-- <el-option label="区域一" value="shanghai1"></el-option> -->
                    <el-option v-for="item in groupinList" :key="item.id" :label="item.name" :value="item.chatId">
                    </el-option>
                  </el-select>
                </el-form-item>
          ...
          <el-form-item :class="ruleForm.resource == 1 ? 'sent_grop' : 'sent_warp'">
            <p class="client_sends">
              预计发送客户数量:
              <span class="sends_num">{{ screenGroupnum || 0 }}</span>
            </p>
          </el-form-item>
          ...
          <el-form-item label="活动时间:" required v-if="ruleForm.regulat == 2" label-width="120px">
            <div class="activity_time">
              <el-form-item prop="timingdata">
                <el-date-picker :picker-options="pickerOption" placeholder="选择日期" v-model="ruleForm.timingdata" class="select_day"
          ...
          <el-form-item>
            <!--  新增素材列表 -->
            <div>
              <!-- <ComponenMaterial  :single="index" :newAddTech="newAddbut" :itemInfo="item" :allList="wordsList" :lode="loading" @fnDelete="fnDelete"
                                @childEvent="childEvent" @getType="getTypeFun" v-for="(item, index) in wordsList" :key="index" /> -->
          ...
          <!-- <el-form-item v-if="this.wordsList.length < 9">
            <div class="flex center_add">
              ...
              <span class="tite_add"><span>（最多可添加 <span>{{ this.wordsList.length }}</span>/
                </span>
...
import { cluecustomerMass_addCustomerScreen } from '@/api/customer'
export default {
  components: {
    GroupAdd,
    ComponenMaterial,
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.wordsList.forEach((item, indexs) => {
        ...
        console.log(item.objList.tab, '-----item.objList.tab')
        let imgArr = []
      ...
      console.log(this.wordsList, '---++12321')
    ...
    //    console.log('asd',index,this.wordsList[index])
    //    let arr = JSON.parse(JSON.stringify(this.wordsList))
    // //   let newsObj = JSON.parse(JSON.stringify(this.wordsList))
    // //   // console.log('2222222222------------', index)
    // //   // if (this.wordsList.length > 1) {
    // //  this.wordsList = newsObj.splice(index, 1)
    // //     console.log( this.wordsList,"----")
    // //   // }
    //   let list = arr.filter((el,i) => {
    //     return i != index
    //   })
    //   this.wordsList = JSON.parse(JSON.stringify(list))
