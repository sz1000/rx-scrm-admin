<template>
  <div class="edit_dialog">
    <el-dialog title="新增标签组" :visible.sync="showAddTag" :before-close="dialogFormCancel"
               :close-on-click-modal='false' width="520px">
      <el-form :model="addCodeForm" :rules="editRules" ref="ruleForm" label-width="100px"
               label-position="right">
        <el-form-item label="标签组名称:" prop="content">
          <el-input show-word-limit v-model.trim="addCodeForm.content" maxlength="30"
                    @change="changeInput" placeholder="请输入标签组名称"></el-input>
        </el-form-item>
        <el-form-item>
          ...
          <Draggable v-model="tagList" @update="datadragEnd" ghostClass="chosen" animation="300">
            <transition-group>
              <div class="tagName" v-for="(item,index) in tagList" :key="index">
                <el-input v-model.trim="item.name" show-word-limit maxlength="30"
                          placeholder="请输入标签名称"></el-input>
                <img src="../../../assets/images/reduceone@2x.png" alt=""
                     @click="reduceOne(item,index)" v-if="tagList.length>1" />
                <img src="../../../assets/images/drag.png" alt="" v-if="tagList.length>1"
...
export default {
  ...
  methods: {
    ...
    dialogFormSure(formName) {
      let params = { name: this.addCodeForm.content, children: this.tagList }
      ...
      if (isTag) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid && this.isName) {
            this.$network
              .post('/customer-service/tag/add', params)
              .then((res) => {
                this.$emit('shutTag')
                this.addCodeForm = {}
              })
