<template>
  <div class="edit_dialog">
    <el-dialog title="新增入群欢迎语" :visible.sync="addChannel" :before-close="dialogFormCancel" :close-on-click-modal="false" width="736px">
      <el-form :model="addCodeForm" :rules="editRules" ref="ruleForm" label-width="84px" label-position="right">
        ...
        <el-form-item label="欢迎语2:">
          <el-upload class="avatar-uploader" action="#" ref="upload" :auto-upload="false" :show-file-list="true" :file-list="fileList"
                     :on-change="changeFile" :on-remove="removeFile" :before-upload="beforeAvatarUpload" :on-preview="handlePictureCardPreview"
        ...
        <el-form-item label="消息提醒:">
          <el-switch v-model="addCodeForm.value" disabled active-value="1" inactive-value="0" active-color="#4168F6" inactive-color="#B9B9B9"
                     @change="changeData">
          </el-switch>
  ...
  methods: {
    ...
    dialogFormSure(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$network
            .post('/customer-service/greeting/add', formData, config)
            .then((res) => {
              this.$emit('closeAddChannel')
              this.addCodeForm = {}
    ...
    handleAvatarSuccess(res, file) {
      // console.log('------Chengg------', this.fileList)
