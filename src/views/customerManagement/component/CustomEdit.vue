<template>
  <div class="edit_dialog">
    <el-dialog title="编辑客户" :visible.sync="customEdit" :before-close="dialogFormCancel" :close-on-click-modal='false' width="736px">
      <!-- 客户信息 -->
      <section>
      ...
      { required: true, message: '请输入客户名称', trigger: ['blur','change'] },
            ...
            <el-input v-model="addCodeForm.customerCalled" @click.native="clickFun" maxlength="13" placeholder="请输入" :readonly="!isWxFriends"></el-input>
          </el-form-item>
          ...
          <el-form-item label="客户阶段:" prop="stage" :rules="[{required: true, message: '请选择', trigger: ['blur']}]">
            <el-select v-model="addCodeForm.stage" placeholder="请选择" @change="customChange" clearable>
              <el-option v-for="item in phaseList" :key="item.value" :label="item.name" :value="item.name">
              </el-option>
            </el-select>
          </el-form-item>
...
import { getlistFiled,cluecustomer_toupdate,cluecustomer_update,cluecustomer_wechat_Contact } from '@/api/customer'
export default {
  components: {
    KehuTip,
    GongsiTip,
  ...
  methods: {
    ...
    clickFun(){ //校验客户名
      if(this.isWxFriends){return false }
      this.$emit('validateName',this.addCodeForm.customerCalled)
    },
    ...
    getData() {
      cluecustomer_toupdate(this.$route.query.customno).then(res => {
        if(res.result){
  ...
  watch: {
    name(val){
      if(val){
        this.addCodeForm.customerCalled = val
      }
    },
  },
}
