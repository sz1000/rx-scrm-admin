      ...
      <el-form :model="addCodeForm"
        ...
        <el-form-item label="标签组名称:"
          ...
          <el-input show-word-limit
                    v-model.trim="addCodeForm.content"
                    maxlength="30"
                    @change="changeInput"
                    placeholder="请输入标签组名称"></el-input>
        </el-form-item>
        <el-form-item>
          ...
          <Draggable v-model="tagList"
            ...
            <transition-group>
              <div class="tagName"
                ...
                <img src="../../../assets/images/reduceone@2x.png"
                     alt=""
                     @click="reduceOne(item,index)"
...
export default {
  ...
  methods: {
    ...
    dialogFormSure(formName) {
      ...
      console.log('222222222----', this.editTagList)
      console.log('222222222----', this.addCodeForm.content)
      let params = {
        ...this.editTagList,
        parentName: this.addCodeForm.content,
        children: this.tagList,
        //name:this.editTagList.content
      }
      ...
      if (isTag) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid && this.isName) {
            this.$network
              .post('/customer-service/grouptag/update', params)
              .then((res) => {
                this.$emit('closeEditTag')
                this.addCodeForm = {}
                this.stagList = []
    ...
    deleteTag() {
      ...
      this.$confirm(
        ...
        .then(() => {
          this.$network
            .get('/customer-service/grouptag/delete', { tagid : this.editTagList.tagid })
            .then((res) => {
              this.$emit('closeEditTag')
              this.$message({
                type: 'success',
