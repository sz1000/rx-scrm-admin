    ...
    <div class="add_wrap">
        ...
        <div class="add_content">
            <el-form :model="detail" :rules="rules" ref="ruleForm" label-width="76px" class="ruleForm">
                <div class="item_wrap">
                    <div class="line_title">创建个人SOP规则</div>
                    <el-form-item label="规则名称：" prop="ruleName">
                        <el-input v-model="detail.ruleName"  maxlength="30" @input="detail.ruleName=detail.ruleName.replace(' ','')" show-word-limit placeholder="请输入规则名称"></el-input>
                    </el-form-item>
                    <el-form-item class="mbt32" label="使用员工：" prop="userIdArr">
                        <el-select class="select" v-model="detail.userIdArr" multiple placeholder="请选择使用员工（可多选）">
                            <el-option
                ...
                <div class="item_wrap">
                                ...
                                <div class="val">
                                    ...
                                    <div class="inside_box" v-for="(son,i) in item.sopContentAttachmentVOS" :key="i">
                                        ...
                                        <!-- <div class="share_link" v-else>{{son.url}}</div> -->
                                           <!-- <div class="share_box" v-if="son.attachmentType == 2 || son.attachmentType == 3 ">
                                            <div>
                                        ...
                                        <div class="share_box" v-if="son.attachmentType == 2 ">
                                         ...
                                         <div class="share_box" v-if="son.tab == 2 && son.attachmentType == 3">
                                                <div class="line">
                                                <div class="img_box">
                                             ...
                                             <div class="img_row" v-if="son.tab == 3 && son.attachmentType == 3">
                                            <div class="img_box">
                                                <img :src="son.url" alt="">
                                            </div>
                                            <div class="info_r">
                    ...
                    <el-button v-if="!isNoData && list && list.length > 0" class="creat_btn" @click="addFun" size="small" type="primary" v-preventReClick>{{id?'保存修改':'创建'}}</el-button>
            ...
            <div class="dialog_content">
                <el-form :model="addData" :rules="addRules" ref="ruleFormAdd" label-width="76px" class="ruleForm">
                    <el-form-item label="内容名称：" prop="name">
                        <el-input v-model="addData.name" @input="addData.name=addData.name.replace(' ','')" maxlength="30" show-word-limit placeholder="请输入内容名称"></el-input>
                    </el-form-item>
                    <el-form-item label="推送时间：" prop="pushType">
                        <div class="d_line">
                            <el-radio v-model="addData.pushType" label="1">新客户添加员工好友</el-radio>
                            ...
                            <span class="text">后提醒员工发送消息</span>
                        </div>
                        <div class="d_line">
                            <el-radio v-model="addData.pushType" label="2">新客户添加员工好友</el-radio>
                    ...
                    <el-form-item label="推送内容：" prop="content">
                        <el-input class="textarea" type="textarea" :autosize="{ minRows: 9}" v-model="addData.content" maxlength="500" show-word-limit placeholder="请输入文字内容"></el-input>
                    </el-form-item>
                    <el-form-item class="hide" label="附件">
                        <div class="enclosure_box" v-loading="loading">
                                ...
                                <div class="val">
                                    ...
                                    <div class="e_line">
                                        ...
                                        <div class="e_val">
                                         <!-- <el-radio-group > -->
                                            ...
                                            <el-radio class="radio" v-model="item.attachmentType" label="2" @change="chaneLink(index)">链接</el-radio>
                                            <el-radio class="radio" v-model="item.attachmentType" label="3" @change="changeRadio(index)">素材库</el-radio>
                                       <!-- </el-radio-group> -->
                                        </div>
                                    </div>
                                        ...
                                        <div class="upload_row">
                                            <div class="upload_box">
                                                ...
                                                <div class="upload" v-else>
                                                    ...
                                                    <input class="file" type="file" @change="uploadFun($event,'images','sop','png,jpg,jpeg','2',index)" accept=".png,.jpg,.jpeg" />
                                                </div>
                                            </div>
                                    ...
                                    <div class="link_wrap" v-if="item.attachmentType == '2'">
                                        <div class="input_item">
                                            <el-input v-model="item.url" @blur="blurUrl"  @keyup.enter.native="onSubmit"  @input="item.url=item.url.replace(/[\u4E00-\u9FA5]|[\uFE30-\uFFA0]|[^\S+$]/g,'')" placeholder="链接地址请以http或https开头"></el-input>
                                        </div>
                                        <div class="l_item">
                                            <div class="l_label">链接标题：</div>
                                            <div class="l_val">  <el-input  v-model="item.title" placeholder=""></el-input></div>
                                        </div>
                                        <div class="l_item">
                                            <div class="l_label">链接摘要：</div>
                                            <div class="l_val"> <el-input  v-model="item.urlAbstract" placeholder=""></el-input></div>
                                        </div>
                                        <div class="l_items">
                                            ...
                                            <div class="upload_box">
                                                ...
                                                <div class="upload" v-else>
                                                    ...
                                                    <input class="file" type="file" @change="uploadFuns($event,'images','sop','png,jpg,jpeg','2',index)" accept=".png,.jpg,.jpeg" />
                                                </div>
                                            </div>
                                            <!-- <div class="afresh_file" v-if="item.url">重新上传<input class="file" type="file" @change="uploadFun($event,'images','sop','png,jpg,jpeg','2',index)" accept=".png,.jpg,.jpeg" /></div> -->
                                                   <div class="afresh_file" v-if="item.cover">重新上传<input class="file" type="file" @change="uploadFuns($event,'images','sop','png,jpg,jpeg','2',index)" accept=".png,.jpg,.jpeg" /></div>
                                                 </div>
                                           </div>
                                        </div>
                                    </div>
                        ...
                        <div class="add_box" v-if="!addData.sopContentAttachmentVOS || (addData.sopContentAttachmentVOS && addData.sopContentAttachmentVOS.length < 9)">
...
import { sop_employeeList,sop_add,sop_detail,sop_edit,sop_sopNameValidate } from '@/api/sop'
export default {
    components: {
        DeleteDialog,
        MaterialDialog
    ...
    data(){
        return {
            ...
            rules: {
                ...
                userIdArr: [
                    { required: true, message: '请选择使用员工', trigger: 'change' },
                ],
            },
            dialog_add: false,
            radio: '1',
    ...
    computed: {
        ...
        isNoData(){   //是否没有可展示数据
            let status = true,list = []
            if(this.detail.sopContentVOS && this.detail.sopContentVOS.length > 0){
                list = this.detail.sopContentVOS.filter(el => {
                    return el.modifyType != 'delete'
                })
            }
            if(list.length > 0){ status = false }
            return status
        },
    },
    mounted(){
    ...
    methods: {
        ...
        fnMaterialInfo(obj) {
            console.log('----obj---', obj)
            let data =   this.addData.sopContentAttachmentVOS[this.deleteIndex]
            console.log(data)
            if(data.attachmentType == 1){
                data.switchover = 1
            }else if(data.attachmentType == 2){
                 data.switchover = 2
            }else if(data.attachmentType == 3){
                 data.switchover = 3
            }else if(obj.tab == 1){
                  data.switchover = 4
            }else if(obj.tab == 2){
     ...
     this.blurUrl()
            ...
            sop_detail(this.id).then(res => {
                if(res.result){
                    ...
                    data.sopContentVOS.forEach(el => {
                        if(!el.content && el.sopContentAttachmentVOS && el.sopContentAttachmentVOS.length > 0){
                    ...
                    arr.forEach(el => {
                        this.detail.userIdArr.push(Number(el))
                    })
                }
            })
        },
        confirmFun(){   //确定添加内容 or 编辑内容
            let data = JSON.parse(JSON.stringify(this.addData))
             console.log(data,"点击确定")
            //  data.sopContentAttachmentVOS.forEach((items,indexs)=>{
            //       if(items.attachmentType == 1){
            //                     items.switchover = 1
            //                 }else if(items.attachmentType == 2){
            //                      items.switchover = 2
            //                 }else if(items.attachmentType == 3){
            //                      items.switchover = 3
            //                 }else if(items.tab == 1){
            //                       items.switchover = 4
            //                 }else if(items.tab == 2){
            ...
            let isEnclosureEmpty = false,isLink = false, material =false
            if(data.sopContentAttachmentVOS && data.sopContentAttachmentVOS.length > 0){
                data.sopContentAttachmentVOS.forEach(el => {
                    console.log(el,"---00")
                    el.content = data.content
                    el.attachmentType = Number(el.attachmentType)
                    if(el.url == '' &&  el.attachmentType == 3 && el.attachmentType == 2 ){
                        isEnclosureEmpty = true
                    }
                    if(el.attachmentType == 2 && (!this.$httpReg(el.url) || !this.$linkReg(el.url))){
                        isLink = true
                    }
                    // if(el.attachmentType == 3){
                    //     material = true
                    // }
                })
            }
            ...
            this.$refs.ruleFormAdd.validate((valid) => {
                if (valid) {
                    ...
                    if(this.editIndex === null){    //新增
                        ...
                        if(modifyData.sopContentAttachmentVOS && modifyData.sopContentAttachmentVOS.length > 0){
                            modifyData.sopContentAttachmentVOS.forEach(el => {
                                if(!el.modifyType){el.modifyType = 'update'}
                            })
                        }
                    }
                    this.dialog_add = false
        ...
        addFun(){   //新增规则确定
         console.log(this.addData)
            ...
            data.userId = data.userIdArr.join('|')
            // data.sopContentVOS.forEach(el => {
            //     el.sopContentAttachmentVOS.forEach(son => {
            //         if(son.attachmentType == 3){
            //             son.attachmentType = 2
            //         }
            //     })
            // })
            ...
            this.$refs.ruleForm.validate((valid) => {
                if (valid) {
                    console.log('submit! valid');
                    this.checkRuleNameFun().then(checkResult => {  //校验是否重复
                        if(checkResult){
                            if(this.id){
                                sop_edit(data).then(res => {
                                    if(res.result){
                                        this.$message({ message: '修改成功！',type: 'success' })
                                        this.$router.replace('/customerManagement/sopProcess/personal')
                                    }
                                })
                            }else{
        ...
        editFun(i){  //编辑内容
            ...
            console.log(data)
            data = Object.assign(data,JSON.parse(JSON.stringify(this.detail.sopContentVOS[i])))
            ...
            if(data.sopContentAttachmentVOS && data.sopContentAttachmentVOS.length > 0){
                data.sopContentAttachmentVOS.forEach(el => {
                    ...
                    // if(el.attachmentType == 1){
                    //     el.attachmentType = 1 
                    // }else if(el.attachmentType == 2){
                    //     el.attachmentType = 2
                    // }else{
                    //      el.attachmentType = 3
                    // }
                    // el.imgUrl = JSON.parse(JSON.stringify(el.url)) || ''
                })
            }
            this.dialog_add = true
        },
        ...
        confirmDeleteFun(){     //确认删除内容
            let i = this.deleteIndex
            let _data = this.detail.sopContentVOS
            if(_data[i].id){
                _data[i].modifyType = 'delete'
                if(_data[i].sopContentAttachmentVOS && _data[i].sopContentAttachmentVOS.length > 0){
        ...
        uploadFuns(e,fileType,type,accept,size,index){   //文件上传
            let fileName = e.target.files[0].name
            let fileSize = e.target.files[0].size
            this.$uploadFun(e,fileType,type,size,accept).then(res => {
                if(res){
        ...
        checkRuleNameFun(){     //校验规则名称是否可用
            let result = true
            if(this.id && (this.sourceRuleName == this.detail.ruleName)){ result = false }
        ...
        getPromptTime(data,val){
            ...
            if(data.pushType == '1'){   //一天内
                if(val){
                    ...
                    str = '新客户添加员工好友' + data.m + util + '后提醒员工发送消息'
                }else{
                    str = data.m + data.timeType
                }
            }else{  //一天以上
                if(val){
                    str = '新客户添加员工好友' + data.d + '天后，当日' + data.time + '后提醒员工发送消息'
                }else{
      ...
      uploadToOss(formData).then((res) => {
        if (res.result) {
             this.addData.sopContentAttachmentVOS.forEach((item,index) =>{
                  item[index].linkImgUrl = res.data.url
             })
        //   this.linkImgUrl = 
