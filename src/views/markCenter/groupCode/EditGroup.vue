<template>
  <div class="edit_dialog">
    <el-dialog title="编辑群活码"
               :visible.sync="editCode"
      ...
      <el-form :model="addCodeForm"
        ...
        <el-form-item label="拉群方式:"
                      prop='addBy'>
          <el-select v-model="addCodeForm.addBy"
                     placeholder="请选择拉群方式"
                     disabled
                     @change="changeWay"
                     clearable>
            <el-option v-for="item in groupStyle"
                       :key="item.value"
                       :label="item.label"
                       :value="item.addBy">
            </el-option>
        ...
        <el-form-item label="开启添加联系人:">
          <el-switch v-model="addCodeForm.switchStatus"
                     active-color="#4168F6"
        ...
        <el-form-item label="使用员工:"
                      prop='name'
                      v-if="addCodeForm.switchStatus">
          <el-select v-model="userArr"
        ...
        <el-form-item label="群名称设置:"
                      v-if='!addCodeForm.switchStatus'>
          <el-input v-model.trim="addCodeForm.groupName"
                    placeholder="请输入活码名称"
                    maxlength="12"
                    show-word-limit></el-input>
        </el-form-item>
        <el-form-item label="入群提示:"
                      v-if='!addCodeForm.switchStatus'>
          <el-input v-model.trim="addCodeForm.addHint"
                    placeholder="请输入入群提示"
                    maxlength="16"
                    show-word-limit></el-input>
        </el-form-item>
        <el-form-item label="入群引导语:"
          ...
          <el-input show-word-limit
                    v-model="addCodeForm.contest"
                    type="textarea"
                    maxlength="300"
                    placeholder="请输入入群引导语"></el-input>
        </el-form-item>
        <div v-for="(item,index) in logoImgs"
             :key="index"
             class="groupBox">
          <span style="color:red;position: absolute;top:8px;left:37px">*</span>
          <el-form-item :label="'二维码'+ (index + 1)+':' ">
            <el-upload class="avatar-uploader"
                       ...
                       :on-change="changeFile.bind(null, {'index':index,'data':item})"
                       :on-remove="removeFile.bind(null, {'index':index,'data':item})"
              ...
              <img v-if="item.imageUrl"
                   ...
                   class="avatar" />
              <div v-else
                   class="textCode"><i class="el-icon-plus avatar-uploader-icon"></i>
                <span>请上传二维码</span>
              </div>
            </el-upload>
            <i class="el-icon-delete"
               v-if="logoImgs.length>1"
               @click="deleteImg(item,index)"></i>
            <div class="maxPeople">
              <span>群上限人数</span>
              <span>
                <el-input-number v-model="item.groupTotal"
            ...
            <div class="selectGroup">
              <el-select v-model="item.chatId"
                         v-if="showRoled && addCodeForm.addBy=='1' "
                         placeholder="选择群聊"
                         @change="changeGroup"
                         clearable>
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
                   @click="dialogFormSure('ruleForm')">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      showRoled: true,
      editRules: {
        name: [{ required: true, trigger: 'blur', message: '请输入活码名称' }],
        region: [{ required: true }],
        contest: [{ required: true, trigger: 'blur', message: '请输入' }],
        addBy: [
  ...
  methods: {
    ...
    getDetail() {
      this.$network
        .get('/user-service/livecodegroup/getByNo', {
          livecodeNo: this.editForm.livecodeNo,
        })
        .then((res) => {
          this.addCodeForm = res.data.livecodeEntity
          this.addCodeForm.region = res.data.livecodeEntity.chname
          this.userArr = res.data.userArr
          let tempList = res.data.grouplist
          this.logoImgs = tempList.map((item) => {
            return {
              imageUrl: item.groupAddress,
      ...
      // console.log('---logoImgs-----', this.logoImgs)
    },
    getGroupInfoList() {
      this.$network
    ...
    dialogFormSure(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid && !isCode) {
          this.$network
            .post('/user-service/livecodegroup/update', params)
            .then((res) => {
              this.$emit('closeEditCode')
              this.addCodeForm = {}
            })
