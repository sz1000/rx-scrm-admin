      ...
      <div style="width: 810px">
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          <el-form-item label="任务名称:" prop="taskName">
            <el-input v-model="ruleForm.taskName" show-word-limit maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="选择群发员工:" class="group" prop="region" label-width="120px">
            <el-select multiple v-model="ruleForm.region" placeholder="请选择群发员工 （可多选）" @change="groupStaff">
              <!-- <el-option label="区域一" value="shanghai"></el-option> -->
              <el-option v-for="(item, index) in groupList" :key="index" :label="item.name" :value="item.userNo">
              </el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="选择群发客户:" prop="groupresource" label-width="120px">
            <el-radio-group v-model="ruleForm.groupresource" @change="groupid">
          ...
          <el-form-item>
            <div v-if="ruleForm.groupresource == 1">
              <div class="screen_group">
                <el-form-item label="性别:">
                  <el-radio-group v-model="sendPerson.gender" @change="radioGender">
                ...
                <el-form-item label="添加时间:">
                  <el-date-picker v-model="datalist" type="daterange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期"
                                  value-format="yyyy-MM-dd" @change="addchangeDate">
                    >
                  </el-date-picker>
                </el-form-item>
                <el-form-item label="所在群聊:">
                  <el-select class="group_name" v-model="ruleForm.group" multiple placeholder="请选择群发员工 （可多选）" @change="groupchat">
                    <!-- <el-option label="区域一" value="shanghai1"></el-option> -->
                    <el-option v-for="(item, index) in groupinList" :key="index" :label="item.name" :value="item.chatId">
                    </el-option>
                  </el-select>
                </el-form-item>
                <el-form-item label="客户标签:">
                  <div class="customer" :class="
                    ...
                    <span class="hint_tite" v-if="customerList == '' || customerList == undefined">请按照标签筛选客户</span>
                    <div class="tage_name" v-for="(item, index) in customerList" :key="index">
                      <span>{{ item.name }}</span>
                      <span class="delete_img pointer" @click.stop="tagtName(item, index)">
                        <img style="width: 12px; height: 12px; margin-left: 5px" src="../../../assets/images/Shutdown.png" alt="" />
                      </span>
                    </div>
                  </div>
                </el-form-item>
          ...
          <el-form-item :class="ruleForm.resource == 1 ? 'sent_grop' : 'sent_warp'">
          ...
          <el-form-item label="活动时间" required v-if="ruleForm.sendType == 2">
            <div class="activity_time">
              <el-form-item prop="date1">
                <el-date-picker type="date" placeholder="选择日期" v-model="ruleForm.timingdata" class="select_day" value-format="yyyy-MM-dd"
                                @change="activityday"></el-date-picker>
              </el-form-item>

              <el-form-item prop="date2">
                <el-time-picker placeholder="选择时间" v-model="ruleForm.timingTime" class="select_time" value-format="HH:mm:ss" @change="activitytime">
                </el-time-picker>
              </el-form-item>
            </div>
          </el-form-item>
          ...
          <el-form-item label="群发内容:">
            <el-input type="textarea" placeholder="欢迎您加入哈哈哈哈哈哈，一起加油吧~" v-model="ruleForm.massContent"></el-input>
          </el-form-item>
          <el-form-item>
            <!--  新增素材列表 -->
            <div class="enclosure_box">
              <div class="e_item" v-for="(item, index) in fujianList" :key="index">
                ...
                <div class="val">
                  ...
                  <div class="e_line">
                    ...
                    <div class="e_val">
                      <el-radio class="radio" v-model="item.type" label="image">图片</el-radio>
                      <el-radio class="radio" v-model="item.type" label="url">链接</el-radio>
                    </div>
                  </div>
                  <div class="upload_wrap" v-if="item.type == 'image'">
                    <div class="upload_row">
                      ...
                      <div class="afresh_file" v-if="item.ossUrl">
                        重新上传<input class="file" type="file" accept=".png,.jpg,.jpeg" />
                      </div>
                    </div>
                  ...
                  <div class="link_wrap" v-else>
                    <div class="input_item">
                      <el-input v-model="item.value" placeholder="链接地址请以http或https开头"></el-input>
                    </div>
                    <!-- <div class="l_item">
...
export default {
  ...
  methods: {
    ...
    getOneCustomerMass() {
      this.$network
        ...
        .then((res) => {
          ...
          res.data.customerMassResourcesEntityList.forEach((item) => {
            //  if(item.type)
          })
          ...
          res.data.userchooseEntityList.forEach((item) => {
            ...
            this.ruleForm.region.push(item.userNo)
            // this.ruleForm.region.push(item.userNo); //
            this.userNoList.push(item.userNo)
          })
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let params = {
            ...
            userList: this.userNoList, //this.grouprst,
            gender: this.gender,
            addStartTime: this.ruleForm.addStarTime,
            addEndTime: this.ruleForm.addEndTime,
            groupList: this.groupListChat, //this.groupDataList || [],
    ...
    Screeningcustomer() {
      console.log(this.userNoList, '--------userNoList')
      console.log(this.customerlistdata, '--------标签')
