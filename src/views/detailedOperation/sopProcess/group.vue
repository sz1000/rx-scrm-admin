  ...
  <div class="list_wrap">
    ...
    <div class="content">
      <div class="btn_box" v-show="expandedKeys.some(item=>item.enName == 'add')">
        <div class="btn_add" @click="$router.push({path:'/detailedOperation/sopProcess/groupAdd',query: {menuFlag: true,type: 1}})">
...
export default {
  ...
  methods: {
    ...
    editFun(row) {
      //编辑
      this.$router.push({
        path: '/detailedOperation/sopProcess/groupAdd',
        query: {
          id: row.id,
        },
      })
    ...
    toDetail(row) {
      //详情
      this.$router.push({
        path: '/detailedOperation/sopProcess/groupDetail',
        query: {
          menuFlag: true,
          type: 1,
          id: row.id,
