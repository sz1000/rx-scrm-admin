  ...
  <div class="list_wrap">
    ...
    <div class="content">
      <div class="btn_box" v-show="expandedKeys.some(item=>item.enName == 'add')">
        <div class="btn_add" @click="$router.push({path:'/detailedOperation/sopProcess/personalAdd',query: {menuFlag: true,type: 1}})">
...
export default {
  ...
  methods: {
    ...
    editFun(row) {
      //编辑
      this.$router.push({
        path: '/detailedOperation/sopProcess/personalAdd',
        query: {
          id: row.id,
        },
      })
    ...
    toDetail(row) {
      //详情
      this.$router.push({
        path: '/detailedOperation/sopProcess/personalDetail',
        query: {
          menuFlag: true,
          type: 1,
          id: row.id,
